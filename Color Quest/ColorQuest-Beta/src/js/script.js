(function() {
	...
	function forceRenderSixBoxes() {
		...
		for (let i=0;i<6;i++) {
			...
			c.style.backgroundColor = col.hex; // Ensure backgroundColor is set
			c.setAttribute('data-color', col.hex); // Store data-color
			c.setAttribute('aria-label', col.name);
			...
			if (level.switchColors) {
				...
				state.colorSwitchIntervalId = setInterval(() => {
					...
					for (let i = 0; i < state.cells.length; i++) {
						const isAnswerCell = state.cells[i].style.background === state.answerColor.hex;
						if (!isAnswerCell && Math.random() < 0.6) {
							const newColor = pickRandom(colorsPool);
							state.cells[i].style.background = newColor.hex;
							state.cells[i].style.backgroundColor = newColor.hex; // Ensure backgroundColor is set
							state.cells[i].setAttribute('data-color', newColor.hex); // Store data-color
							state.cells[i].setAttribute('aria-label', newColor.name);
						}
					}
				}, switchMs);
			}
	...
	btnHint.addEventListener('click', () => {
		if (state.usedHint || !state.isPlaying) { showToast('Hint unavailable'); return; }
		state.usedHint = true;
		btnHint.disabled = true;
		const answerHex = state.answerColor.hex;
		state.cells.forEach(el => {
			if (el.style.background === answerHex) {
